...
public class CropBatchServiceImpl implements CropBatchService {
    ...
    public Result add(CropBatch cropBatch) {
        ...
        List<Lot> batchLotList = lotMapper.selectByBaseIdAndYear(cropBatch.getBaseId(),cropBatch.getStartYear().intValue(),null,cropBatch.getCropId());
        ...
        //添加/修改地块规模
        for (LotCalRel lotCalRel : cropBatch.getLotCalRelList()) {
            lotCalRel.setCropId(cropBatch.getCropId());
            if (lotCalRel.getId() == null){
                lotCalRelMapper.insertSelective(lotCalRel);
            }else{
    ...
    public Result update(CropBatch cropBatch) {
        ...
        try {
            List<Lot> lotList = cropBatch.getLotList();
            List<Lot> batchLotList = lotMapper.selectByBaseIdAndYear(cropBatch.getBaseId(),cropBatch.getStartYear().intValue(),cropBatch.getId(),cropBatch.getCropId());
            StringBuffer sb = new StringBuffer();
            boolean isSuccess = true;
            if (batchLotList != null){
                for (Lot lot : batchLotList) {
