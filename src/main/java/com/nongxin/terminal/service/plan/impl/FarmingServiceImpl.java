...
import com.nongxin.terminal.dao.plant.FarmingCalRelMapper;
...
public class FarmingServiceImpl implements FarmingService {
    ...
    @Autowired(required = false)
    private FarmingCalRelMapper farmingCalRelMapper;
    ...
    public boolean add(Farming farming) {
        try {
            ...
            if (farming.getCalId() != null){
                farmingCalRelMapper.insertSelective(new FarmingCalRel(farming.getId(),farming.getCalId()));
            }
            return true;
        }catch (Exception e){
            e.printStackTrace();
    ...
    public boolean update(Farming farming) {
        try {
            ...
            if (farming.getCalId() != null){
                farmingCalRelMapper.deleteByFarmingId(farming.getId());
                farmingCalRelMapper.insertSelective(new FarmingCalRel(farming.getId(),farming.getCalId()));
            }
            return true;
        }catch (Exception e){
