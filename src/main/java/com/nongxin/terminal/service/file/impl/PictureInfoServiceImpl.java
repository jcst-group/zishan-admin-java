...
public class PictureInfoServiceImpl implements PictureInfoService {
    ...
    @Override
    @Transactional(rollbackFor = Exception.class)
    public Result uploadPic(MultipartFile file, Integer relationId, ImgRelationTypeEnum imgRelationTypeEnum) {
        Result<PictureInfo> result = new Result<>();
        Result<FileInfoVo> result1 = imgUpload.uploadPic(file);
        if (result1.isSuccess()){
            try {
                PictureInfo pictureInfo = new PictureInfo();
                pictureInfo.setPictureUrl(result1.getResult().getPath());
