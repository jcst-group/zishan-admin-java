...
//    }
    ...
    @GetMapping("/getWarnRecord2")
    @ApiOperation(value = "获取预警记录信息")
    public Result<PageInfo> getWarnRecord2(@RequestParam(name="pageNo", defaultValue="1") Integer pageNo,
                                           @RequestParam(name="pageSize", defaultValue="5") Integer pageSize,String[] baseId,Integer typeId){
        Result<PageInfo> result = new Result<>();
        PageHelper.startPage(pageNo,pageSize);
        List<WarnRecord> recordList = warnRecordService.getWarnRecordByTime(typeId,baseId);
        PageInfo pageInfo = new PageInfo(recordList);
    ...
    @GetMapping("/getSpecialByTime")
    @ApiOperation(value = "获取近7天/30天的数据")
    public Result<SpecialVo> getSpecialByType(Integer typeId,Integer index,Integer baseId){
        Result<SpecialVo> result = new Result<>();
        SpecialVo specialVo = envDayAvgService.getSpecialByType(typeId,index,baseId);
        result.setSuccessResult(specialVo);
        return result;
    }
}
