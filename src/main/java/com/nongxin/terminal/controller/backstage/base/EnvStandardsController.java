...
public class EnvStandardsController {
    ...
    public Result addEnvStandards(@Validated @RequestBody EnvStandards envStandards){
        ...
        List<EnvStandards> list = envStandardsService.selectByItem(envStandards.getItem(),envStandards.getStandardTypeId(),null);
        if (list != null && !list.isEmpty()){
            result.setSuccess(false);
            result.setMessage("该检测项目已存在，请勿重复添加");
            return result;
        }
    ...
    public Result updateEnvStandards(@RequestBody EnvStandards envStandards){
        Result result = new Result();
        List<EnvStandards> list = envStandardsService.selectByItem(envStandards.getItem(),envStandards.getStandardTypeId(),envStandards.getId());
        if (list != null && !list.isEmpty()){
            result.setSuccess(false);
            result.setMessage("该检测项目已存在，请勿重复添加");
            return result;
    ...
    @ApiOperation("根据模板id查询检测项")
    public Result selectByTemplateId(@RequestParam(required = true) Integer templateId){
        Result result = new Result();
        result.setSuccessResult(envStandardsService.selectByTemplateId(templateId));
        return result;
    }
