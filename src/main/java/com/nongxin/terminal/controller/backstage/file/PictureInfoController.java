...
public class PictureInfoController {
    ...
    @ApiOperation("添加文章图片")
    public Result addArticlePic(MultipartFile file,Integer recordId){
        return pictureInfoService.uploadPic(file,recordId, ImgRelationTypeEnum.ARTICLE);
    }

    @PostMapping("/addExpectPic")
    @ApiOperation("添加预期愿景图片")
    public Result addExpectPic(MultipartFile file,Integer recordId){
        return pictureInfoService.add(file,recordId, ImgRelationTypeEnum.EXPECT);
    }

    @PostMapping("/addPesticideResiduesPic")
    @ApiOperation("添加农残图片")
    public Result addPesticideResiduesPic(MultipartFile file,Integer recordId){
        return pictureInfoService.add(file,recordId, ImgRelationTypeEnum.PESTICIDE_RESIDUES);
    }

    @PostMapping("/addSugarContentPic")
    @ApiOperation("添加糖度图片")
    public Result addSugarContentPic(MultipartFile file,Integer recordId){
        return pictureInfoService.add(file,recordId, ImgRelationTypeEnum.BRIX);
    }

    @PostMapping("/addFruitReportPic")
    @ApiOperation("添加果品报告图片")
    public Result addFruitReportPic(MultipartFile file,Integer recordId){
        return pictureInfoService.add(file,recordId, ImgRelationTypeEnum.FRUIT_REPORT);
    }
    ...
    public ResponseEntity showCompressionPic(@PathVariable String fileName){
        try {
            /*float quality = 1f;
            File file = new File(imgUpload.getPath(fileName));
