...
public class BaseInfoController {
    ...
    @ApiOperation("添加基地检测数据")
    public Result addDetection(@RequestBody BaseInfo baseInfo){
        Result result = new Result();
        boolean isSuccess = baseInfoService.addDetection(baseInfo);
        result.setMessage(isSuccess ? "添加基地检测成功" : "添加基地检测失败");
        result.setSuccess(isSuccess);
        return result;
    }
