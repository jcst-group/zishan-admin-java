...
public class FileUpload {
    ...
    public Result<FileInfoVo> listSample(MultipartFile file) {
        ...
        if(!isFile){
            result.error500("文件不为doc,docx,pdf,mp3,wav,ogg,mp4,amr,txt,xlsx,zip，无法上传");
            result.setMessage("文件不为doc,docx,pdf,mp3,wav,ogg,mp4,amr,txt,xlsx,zip，无法上传");
            return result;
        }
        ...
        try {
            ...
            if (suffixName.equals("amr")){
                ...
                if (suffixName.equals("mp3")){
                    ...
                    duration = new BigDecimal(multimediaObject.getInfo().getDuration()).divide(new BigDecimal(1000),0,BigDecimal.ROUND_HALF_UP);
                    System.out.println(multimediaObject.getInfo());
                }
            }*/
            File saveFile = new File(savePath);
            FileCopyUtils.copy(file.getBytes(), saveFile);
