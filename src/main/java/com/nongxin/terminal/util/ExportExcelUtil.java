...
public class ExportExcelUtil {
    ...
    public void retroCodingExport(RetroCodingTypeEnum type, String batchCode, String retroCode, HttpServletResponse response){
        ...
        List<RetroCoding> list = retroCodingService.getRetroCoding(RetroCodingTypeEnum.RECO_BATCH,batchCode,retroCode);
        SimpleDateFormat simpleDateFormat = new SimpleDateFormat("yyyy-MM-dd"); //加上时间

        // excel标题
        String[] title = { "追溯码", "打码时间", "品种名称","批次编码", "药材规格","产地","质检员","注意事项"};
        ...
        for (int i = 0; i < list.size(); i++) {
            ...
            content[i][4] = retroCoding.getRetroCodeInfo().getSpecifications();
            content[i][5] = retroCoding.getRetroCodeInfo().getProvenance();
            content[i][6] = retroCoding.getRetroCodeInfo().getQc();
            content[i][7] = retroCoding.getRetroCodeInfo().getRemarks();
        }
        HSSFWorkbook wb = ExportExcelUtil.getHSSFWorkbook(title, content);
