...
import com.nongxin.terminal.module.shiro.ShowToken;
 ...
 **/
@Slf4j
public class ShowFilter extends BasicHttpAuthenticationFilter {

	private static final Logger LOGGER = LoggerFactory.getLogger(ShowFilter.class);
	...
	protected boolean isAccessAllowed(ServletRequest request, ServletResponse response, Object mappedValue) {
		LOGGER.info("isAccessAllowed：showfile");
	...
	protected boolean executeLogin(ServletRequest request, ServletResponse response) throws Exception {
		...
		ShowToken showToken = new ShowToken(token);
		// 提交给realm进行登入，如果错误他会抛出异常并被捕获
		getSubject(request, response).login(showToken);
