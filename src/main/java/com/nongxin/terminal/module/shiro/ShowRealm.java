 ...
 **/
@Component
public class ShowRealm extends AuthorizingRealm {

	private static final Logger LOGGER = LoggerFactory.getLogger(ShowRealm.class);
	...
	public boolean supports(AuthenticationToken token) {
		return token instanceof ShowToken;
	}
	...
	protected AuthorizationInfo doGetAuthorizationInfo(PrincipalCollection principals) {
		...
		LOGGER.info("进入 ShowRealm doGetAuthorizationInfo，鉴权验证");

		SimpleAuthorizationInfo info = new SimpleAuthorizationInfo();
		if(principals!=null) {
			User user =  (User) principals.getPrimaryPrincipal();
			List<ShowResource> showResourceList = showResourceService.getShowResourceByUid(user.getUid(), ResourceTypeEnum.FRONT_RECOURCE);
			Set<String> permissionSet = new HashSet<>();
			for(ShowResource a:showResourceList){
				if (a.getShowResourceRelList()!=null && !a.getShowResourceRelList().isEmpty()){
					for (ShowResourceRel resourceRel : a.getShowResourceRelList()){
						permissionSet.add(resourceRel.getUrl());
					}
				}
	...
	protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken auth) throws AuthenticationException {
		...
		LOGGER.info("进入 ShowRealm doGetAuthenticationInfo，登陆验证");
	...
	public boolean jwtTokenRefresh(String token, String userName, String passWord) {
		String cacheToken = String.valueOf(redisUtil.get(CommonConstant.PREFIX_SHOW_TOKEN + token));
		if (oConvertUtils.isNotEmpty(cacheToken)) {
			//校验token有效性
			if (!JwtUtil.verify(token, userName, passWord)) {
				String newAuthorization = JwtUtil.sign(userName, passWord);
				redisUtil.set(CommonConstant.PREFIX_SHOW_TOKEN + token, newAuthorization);
				 //设置超时时间
				redisUtil.expire(CommonConstant.PREFIX_SHOW_TOKEN + token, JwtUtil.EXPIRE_TIME/1000);
			} else {
				redisUtil.set(CommonConstant.PREFIX_SHOW_TOKEN + token, cacheToken);
				 //设置超时时间
				redisUtil.expire(CommonConstant.PREFIX_SHOW_TOKEN + token, JwtUtil.EXPIRE_TIME/1000);
			}
			return true;
