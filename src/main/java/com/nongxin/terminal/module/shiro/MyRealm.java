...
public class MyRealm extends AuthorizingRealm {
	...
	protected AuthorizationInfo doGetAuthorizationInfo(PrincipalCollection principals) {
		...
		//LOGGER.info("进入 MyRealm doGetAuthorizationInfo，鉴权验证");

		SimpleAuthorizationInfo info = new SimpleAuthorizationInfo();
		if(principals!=null) {
			User user =  (User) principals.getPrimaryPrincipal();
			List<Resource> resourcesList = resourceService.getResourceByUid(user.getUid(), ResourceTypeEnum.FRONT_RECOURCE);
			Set<String> permissionSet = new HashSet<>();
			for(Resource a:resourcesList){
				if (a.getResourceRelList()!=null && !a.getResourceRelList().isEmpty()){
					for (ResourceRel resourceRel : a.getResourceRelList()){
	...
	protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken auth) throws AuthenticationException {
		...
		//LOGGER.info("进入 MyRealm doGetAuthenticationInfo，登陆验证");
